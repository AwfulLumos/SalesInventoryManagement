        ...
        <div className="bg-card rounded-lg shadow p-4 border border-border/60">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-primary">{activeCustomers}</p>
            </div>
        ...
        <div className="bg-card rounded-lg shadow p-4 border border-border/60">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-primary">{totalLoyaltyPoints}</p>
            </div>
            ...
            {filteredCustomers.map((customer) => (
              <button
                ...
                className={`w-full text-left p-4 rounded-lg border border-border transition-colors ${
                  selectedCustomer === customer.id ? 'border-primary/40 bg-primary/10' : 'hover:bg-muted/40'
                }`}
              >
            ...
            <div>
              ...
              <div className="space-y-4 mb-6">
                ...
                <div className="bg-secondary/30 border border-secondary/50 rounded-lg p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm text-secondary-foreground">Loyalty Points</p>
                      <p className="text-3xl font-bold text-primary">{selectedCustomerData.loyaltyPoints}</p>
                    </div>
                    <Award className="text-primary" size={48} />
                  </div>
                  <p className="text-xs text-secondary-foreground mt-2">Redeem 1000 points for ₱100 discount</p>
                </div>

                <div className="bg-primary/10 border border-primary/30 rounded-lg p-4">
                  <p className="text-sm text-secondary-foreground">Total Purchases</p>
                  <p className="text-3xl font-bold text-primary">₱{selectedCustomerData.totalPurchases.toLocaleString()}</p>
              ...
              <div className="border-t pt-4">
                ...
                {selectedCustomerTransactions.length > 0 ? (
                  ...
                  <p className="text-sm text-muted-foreground/80">No purchase history</p>
                )}
              </div>
            </div>
          ) : (
